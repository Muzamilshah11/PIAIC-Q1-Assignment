...
const fav_fruit = ['Banana', 'Mango', 'Cherry', 'WaterMelon'];
...
// Task: 28 ############### Hello Admin ####################
const hello_arry = ['user1', 'user2', 'user3', 'admin', 'user4'];
...
const current_user = ['Ali', 'khan', 'Ahmad', 'Luqman'];
const new_user = ['Shahab', 'Salman', 'Akmal', 'Adil'];
current_user.forEach((check_new_user) => {
    const lowerCase_current_user = current_user.map((curr_user) => curr_user.toLowerCase());
    const lower_new_user = new_user.map((new_user) => new_user.toLowerCase());
